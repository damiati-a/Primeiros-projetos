# Analisador completo de dados de pessoas

# A media da idade do grupo
# O nome do homem mais velho
# quantidade de mulheres com menos de 20 anos
# quantidade de pessoas de outro sexo


somaidade = 0
mediaidade = 0
maioridadehomem = 0
nomevelho = 0
totmulher20 = 0
totoutro = 0

for p in range(1, 5):
    print('--------- {}ª PESSOA ---------'.format(p))
    nome = str(input('Nome: ')).strip()
    idade = int(input('Idade: '))
    sexo = str(input('Sexo [M/F] ou Outro: ')).strip()
    somaidade += idade

    if p == 1 and sexo in 'Mm':
        maioridadehomem = idade
        nomevelho = nome
    if sexo in 'Mm' and idade > maioridadehomem:
        maioridadehomem = idade
        nomevelho = nome
    if sexo in 'Ff' and idade > 20:
        totmulher20 += 1
    if sexo in 'OUTROOutrooutro':
        totoutro += 1

mediaidade = somaidade / 4
print('\n')
print('A média de idade do grupo é de {} anos'.format(mediaidade))
print('O homem mais velho do grupo tem {} anos e se chama {}.'.format(maioridadehomem, nomevelho))
print('Ao todo são {} mulheres com menos de 20 anos de idade.'.format(totmulher20))
print('O total de pessoas que se autodeclaram de outro sexo são de {}.'.format(totoutro))
